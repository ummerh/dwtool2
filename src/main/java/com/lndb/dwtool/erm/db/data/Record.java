...
public class Record {
	...
	public String toHtmlRow(String formId, String fkName, String columnName) {
		...
		String html = "<tr><td>";
		if (isFromLookup()) {
			String returnLink = "<a href='" + "FIXME" + "/maintenanceAction!refreshFromLookup.action?connectionName="
					+ connectionName + "&tableName=" + tableDescriptor.getTableName() + "&recordId=" + recordId
					+ "&fkName=" + fkName + "&formId=" + formId + "&columnName=" + columnName + "'>" + "Return</a>";
			html = html + returnLink + "</td>";

		} else {
			String editLink = "<a href='" + "FIXME" + "/maintenanceAction!editStep2.action?connectionName="
					+ connectionName + "&tableName=" + tableDescriptor.getTableName() + "&formId=" + formId
					+ "&recordId=" + recordId + "'>" + "Edit</a>";

			String deleteLink = "<a href='" + "FIXME" + "/maintenanceAction!deleteRecord.action?connectionName="
					+ connectionName + "&tableName=" + tableDescriptor.getTableName() + "&formId=" + formId
					+ "&recordId=" + recordId
					+ "' onclick=\"return confirmDelete('Please click OK to delete this record.')\" >" + "Delete</a>";
