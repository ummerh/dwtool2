...
public class ExcelProcessor implements FileProcessor {
	 ...
	 */
	public void process(Connection con, FileStatistics statistics, String connectionName,
			TableDescriptor tableDescriptor, File dataFile, boolean headerIncluded, SchemaJoinMetaData joinMetaData,
			File good, File bad) {
				...
				if (dataCols.length != refheaders.length) {
					...
					if (tableRow.validateDataTypes()) {
						if ("FAIL".equals(tableRow.checkDataIntegrity(con, joinMetaData))) {
							badRowCnt++;
							writeBadLine(statistics, badWb, badSheet, row, badRowCnt, refheaders,
									tableRow.getErrorsFound());
						} else {
							goodRowCnt++;
	...
	private void writeGoodLine(FileStatistics statistics, Workbook wb, Sheet goodSheet, Row currRow, short rowNum,
			String[] headers) {
	...
	private void writeBadLine(FileStatistics statistics, Workbook wb, Sheet badSheet, Row currRow, short rowNum,
			String[] headers, List<String> errors) {
	...
	private void writeLine(Sheet sheet, Row currRow, short rowNum, Workbook wb, List<String> errors) {
		...
		if (errors != null && !errors.isEmpty()) {
			...
			Cell newCell = newRow.createCell(pos, CellType.NUMERIC);
	...
	private String[] validatedHeaders(boolean headerIncluded, FileStatistics statistics, Row headerRow,
	 ...
	 */
	public void performDBUpdates(FileStatistics statistics, String connectionName, TableDescriptor tableDescriptor,
			File goodData) {
		Connection con = null;
		FileInputStream dataInputStream = null;
		try {
			...
			Object[] dataCols = null;
			PreparedStatement updtStmt = con
					.prepareStatement(TableRow.prepareUpdateStatement(headers, tableDescriptor));
			PreparedStatement insertStmt = con
					.prepareStatement(TableRow.prepareInsertStatement(headers, tableDescriptor));
			Row row = null;
			while (rows.hasNext()) {
				row = rows.next();
	...
	public FileStatistics process(String connectionName, TableDescriptor tableDescriptor, File dataFile,
			boolean headerIncluded, SchemaJoinMetaData joinMetaData, File good, File bad) {
		...
		try {
			con = DatabaseConnection.newConnection(connectionDetail);
			process(con, statistics, connectionName, tableDescriptor, dataFile, headerIncluded, joinMetaData, good,
					bad);
		} catch (Exception e) {
			throw new RuntimeException(e);
		} finally {
