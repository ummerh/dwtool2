 ...
 * @author harsha07
 * 
 *         This class should be able to compare a base schema to new target
 *         schema and then compare against latest OJB definition
 * 
 *         Report should list the following
 * 
 *         <li>Tables to be removed from base</li>
 *         <li>Tables to be added to base</li>
 *         <li>Tables with same name but unmatched definition</li>
 *         <li>Foreign Keys to be dropped</li>
 *         <li>Foreign keys to be added</li>
 *         <li>Constraints to be dropped</li>
 *         <li>Constraints to be added</li>
 * 
 */
public class DBDiffGenerator {
	...
	protected static final Pattern SIZE_DECR_EXCLUDE_PATTERN = Pattern
			.compile(Configuration.getProperty("report.col.size.decrease.exclude.pattern"));
	protected static final Pattern TYPE_CHANGED_EXCLUDE_PATTERN = Pattern
			.compile(Configuration.getProperty("report.col.type.changed.exclude.pattern"));
	protected static final Pattern DEFAULT_CHANGED_EXCLUDE_PATTERN = Pattern
			.compile(Configuration.getProperty("report.col.default.changed.exclude.pattern"));
	protected static final Pattern TECH_COLS_EXCLUDE_PATTERN = Pattern
			.compile(Configuration.getProperty("report.technical.columns.exclude.pattern"));
	...
	public int generateTableDiffReport(String mismatchTbl, int rowCnt, HSSFSheet sheet) {
		...
		for (ColumnDescriptor tgtCol : toCols) {
			...
			if (srcCol == null) {
				...
				if (!tgtCol.isDefualtEqual(srcCol)
						&& !DEFAULT_CHANGED_EXCLUDE_PATTERN.matcher(tgtCol.getName()).matches()) {
					defaultChanged = true;
				}

			}
			if (addCol || colSizeIncr || colSizeDecr || dataTypChanged || changeToNullable || changeToNonNullable
					|| defaultChanged) {
				Row newRow = sheet.createRow(++rowNum);
	...
	protected void writeFKList(BufferedWriter writer, String header, List<ForeignKey> list) throws IOException {
		...
		for (ForeignKey key : list) {
			writer.write(key.getReferByTable() + "  " + key.getName() + "  " + key.getReferByCols().toString()
					+ "   >>   " + key.getReferToTable() + "  " + key.getReferToCols());
			writer.newLine();
		}
		writer.write("----------------------------------------------------------------------------------------");
		writer.newLine();
	...
	protected void writeOJBRelationsAddedSQL(BufferedWriter out) throws IOException {
		...
		for (ForeignKey constraint : this.ojbKeysToBeAdded) {
			if (constraint.isValid()) {
				counter++;
				out.write(DDL.addConstraintDDL(constraint.getReferByTable(),
						buildConstraintName("O", constraint.getReferByTable(), "FK", counter), constraint));
				out.newLine();
			}
		}
	}
	...
	protected void writeTablesAddedSQL(BufferedWriter out) throws IOException, InvalidMapException {
		DDLGenerator ddlGenerator = new DDLGenerator(this.targetDb);
	...
	protected void writeMismatchedTableColSQL(BufferedWriter out) throws IOException {
		...
		for (String mismatchTblNm : this.mismatchedTables) {
			out.write(getBaseDBDialect().generateTableDiffDLL(this.baseDb.getTableDescriptor(mismatchTblNm),
					this.targetDb.getTableDescriptor(mismatchTblNm)));
		}
		out.newLine();
	}
