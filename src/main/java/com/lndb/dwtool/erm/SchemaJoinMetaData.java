...
public class SchemaJoinMetaData {
	 ...
	 * relations
	 * 
	 * @param baseMap   primary relational map
	 * @param targetMap secondary relational map
	 ...
	 * @param foreignKeys
	 */
	private void iterateForeignKeys(RelationalMap baseMap, RelationalMap targetMap, String table,
	...
	public List<ForeignKey> getImportedKeys(String tableName) {
		...
		if (targetKeys != null) {
			for (ForeignKey foreignKey : targetKeys) {
				if (foreignKey.isValid() && !foreignKey.getReferToTable().equals(tableName)
						&& !isDuplicateFK(foreignKey) && !isDuplicateFK(importedKeys, foreignKey)) {
					importedKeys.add(foreignKey);
				}
			}
		}
	...
	/**
	 * Checks data integrity for the whole schema using the available relationships
	 * 
	 * @param con
	 * @return
	 */
	 ...
	 * @throws IOException
	 */
	public void printDataIntegrityErrors(Connection con, BufferedWriter writer, String tableName)
				...
				try {
					...
					while (result != null && result.next()) {
						if (trackkeys.add(secondaryKey)) {
							writer.newLine();
							writer.write("****** RELATIONSHIP VIOLATED WITH TABLE - " + secondaryKey.getReferToTable()
									+ " " + secondaryKey.getReferByCols().toString() + " *******");
							writer.newLine();
						}
	...
	private void printError(BufferedWriter writer, Exception e, String existenceCheckSql) throws IOException {
		writer.write(
				"Error occured, mostly due to columns missing or column mismatch. Check if DB table version matches OJB definition.");
		writer.newLine();
	...
	private boolean shouldCheckReference(ForeignKey fk, List<String> referToCols, List<String> referByCols) {
		return !fk.isCascadeUpdate() && !isDuplicateFK(fk) && !fk.isReferToView() && referToCols != null
				&& !referToCols.isEmpty() && referByCols != null && !referByCols.isEmpty()
	...
	public String checkDataIntegrity(Connection con, String tableName) {
		try {
			...
			for (ForeignKey secondaryKey : secondaryKeys) {
				...
				List<String> referByCols = secondaryKey.getReferByCols();
				if (shouldCheckReference(secondaryKey, referToCols, referByCols)) {
					String existenceCheckSql = generateExistenceCountSql(tableName, secondaryKey, referToCols,
							referByCols);
	 ...
	 * @return
	 */
	private String generateExistenceCountSql(String tableName, ForeignKey secondaryKey, List<String> referToCols,
			List<String> referByCols) {
	 ...
	 * @return
	 */
	private String generateExistenceDataSql(String tableName, ForeignKey secondaryKey, List<String> referToCols,
			List<String> referByCols) {
		StringBuilder sqlBuf = new StringBuilder();
